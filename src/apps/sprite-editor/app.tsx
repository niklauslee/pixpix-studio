...
import { findSprite, parseSpriteSet, serializeSpriteSet } from "./sprite";
...
interface SpriteEditorUser {
  name: string;
  image?: string | null;
}

interface InitialSpriteSet {
  id: string;
  name: string;
  data: string;
}
...
function App({
  ...
  user: SpriteEditorUser | null;
  initialSpriteSet: InitialSpriteSet | null;
}) {
  ...
  const hover = useSpriteStore((state) => state.hover);
  const setProject = useSpriteStore((state) => state.setProject);
  const [notice, setNotice] = useState("");
  const [savedId, setSavedId] = useState<string | null>(
    initialSpriteSet?.id ?? null,
  );
  const [savedName, setSavedName] = useState(initialSpriteSet?.name ?? "");
  ...
  // load the project passed down from the dashboard (`/sprite?id=...`), once
  useEffect(() => {
    if (!initialSpriteSet) return;
    try {
      const loaded = parseSpriteSet(initialSpriteSet.data);
      setProject(loaded);
      savedProjectRef.current = loaded;
    } catch (error) {
      console.error("Failed to load the saved sprite set:", error);
      flash("Failed to load the saved sprite set");
    }
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const data = serializeSpriteSet(project);
      if (savedId) {
        await api(`/api/sprite-sets/${savedId}`, {
          method: "PATCH",
        ...
        history.replaceState(null, "", `/sprite?id=${created.id}`);
      }
      savedProjectRef.current = project;
      flash("Saved to your account");
    } catch (error) {
      console.error("Failed to save the sprite set:", error);
      flash("Save failed");
    } finally {
      setSaving(false);
    }
  ...
  // keyboard shortcuts — bitmap operations act on the selected sprite
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      const current = findSprite(store.project, store.name);
      ...
      <main className="absolute inset-0 flex select-none flex-col bg-background text-foreground">
        <Appbar active="sprite">
          ...
          {user ? (
            <Button
              ...
              title={
                savedId
                  ...
                  : "Save this sprite set to your account"
              }
            ...
            <a
              href="/login"
              title="Sign in to save sprite sets to your account"
              ...
              <div className="flex min-h-full min-w-full items-center justify-center p-6">
                {sprite ? (
                  ...
                  <div className="text-xs text-muted-foreground/60">
                    No sprite selected — add one from the sprite browser
                  </div>
                )}
              </div>
            </div>
